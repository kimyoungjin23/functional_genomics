...
sum(ave.counts == 0)
hist(log10(ave.counts2), breaks=100, main="", col="grey80", 
    xlab=expression(Log[10]~"average count"))
sum(ave.counts == 0)
```
...
There are 192 plates?
```{r}
# your code here
length(rownames(colData(sce)))
...
```{r}
# your code here
sum(grepl("Mt",rownames(counts(sce))))
```
...
```{r}
# your code here
sum(libsize.drop) + sum(feature.drop) + sum(spike.drop)
```
...
```{r}
# your code here
ave.counts2 <- calcAverage(sce[,keep], use_size_factors=FALSE)
num.cells2 <- nexprs(sce[,keep], byrow=TRUE)
smoothScatter(log10(ave.counts2), num.cells2, ylab="Number of cells", 
    xlab=expression(Log[10]~"average count"))
```
